...
class DIW(Dataset):
    def __init__(self, root_dir, is_train=True, num=0, transforms=None):
        super(DIW, self).__init__()
        self.is_train = is_train
        self.num = num
        self.transforms = transforms
        # load the list of diw images
        self.X = glob.glob(os.path.join(root_dir, 'img', '*'), recursive=True)
        self.X = [fp for fp in self.X if re.search('.*.(jpg|png)', fp)]
    ...
    def __getitem__(self, index):
        ...
        if self.transforms:
            im = self.transforms['im_transforms'](im.copy())
            ms = self.transforms['msk_transforms'](ms.copy())
            ms = (ms > 0.5) * 1.0
