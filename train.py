...
if args['G_ckpt']:
    netG.load_state_dict(torch.load(args['G_ckpt'], map_location=torch.device('cpu'))['G'])
if args['L_ckpt']:
    netL.load_state_dict(torch.load(args['L_ckpt'], map_location=torch.device('cpu'))['L'])
...
trn_loader = DataLoader(DIW(root_dir=args['diw_root'], transforms=make_transforms()), num_workers=4, batch_size=8, shuffle=True, pin_memory=True)
...
def train_L_step_w(engine, batch):
    ...
    dump_xdh = torch.randn(x.size(0), 64, 64)
    tgs, invtgs = warpUtil.perturb_warp(dump_xdh)
...
# trainer.add_event_handler(Events.EPOCH_COMPLETED(every=2), ckpt_g_w, netG)
# trainer.add_event_handler(Events.EPOCH_COMPLETED(every=2), ckpt_l_w, netL)
